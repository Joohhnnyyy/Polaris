...
function AutopsyPage() {
  ...
  async function fetchData(currentIssueId: string | null) {
    setLoading(true);
    try {
      const [clustersData, issuesData] = await Promise.all([
        safeFetchArray<any>(`${API_URL}/clusters`),
        safeFetchArray<any>(`${API_URL}/issues`)
      ]);
      ...
      if (!targetIssueId && Array.isArray(issuesData) && issuesData.length > 0) {
        targetIssueId = issuesData[0].id;
        setSelectedIssueId(targetIssueId);
      }

      if (targetIssueId && Array.isArray(issuesData)) {
        const found = issuesData.find((i: any) => i.id === targetIssueId);
                ...
                <div className="mt-4 mb-6 flex items-center gap-3">
                  ...
                  <div className="relative w-full max-w-[280px]">
                    <select 
                      ...
                      className="appearance-none w-full border border-white/10 hover:border-white/20 rounded-lg pl-3 pr-10 py-2 text-xs text-slate-200 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent cursor-pointer shadow-lg transition-all duration-200"
                      style={{ backgroundColor: "#111318" }}
                    >
                      <option value="" style={{ backgroundColor: "#111318", color: "#94a3b8" }}> Select Incident Case </option>
                      {issues.map((issue) => (
                        <option key={issue.id} value={issue.id} style={{ backgroundColor: "#111318", color: "#f1f5f9" }}>
